...
func (re *replicaEntity) Activate() {
	...
	err := dispatcher.Event(re.env.PluginPartition(), now, proto.EventType_CREATE, &skplug.Pod{
		Name: string(re.Name()),
...
func (re *replicaEntity) Deactivate() {
	now := re.env.CurrentMovementTime().UnixNano()
	err := dispatcher.Event(re.env.PluginPartition(), now, proto.EventType_DELETE, &skplug.Pod{
		Name: string(re.Name()),
	})
	if err != nil {
		panic(err)
